...
#+BEGIN_SRC gradle
   ...
   compile codeProject(':access-control-api')

   include "access-control-api"

   codeProject(':access-control-api').projectDir = "$rootDir/access-control-api" as File
...
All the codeFields are able to be searched with different operators.
...
| f_[codeEntity].[field name]_op=[value] | f_nestedEntity.id_op | search id of nestedentity |
